...
fn evaluate(tokens: &[Token]) -> (usize, u64) {
    let mut total = 0;
    let mut index = 0;
    ...
    while index < tokens.len() {
        ...
        match token {
            ...
            Token::ExpressionStart => {
                let (increment, n) = evaluate(&tokens[index + 1..]);
                total = apply(total, n, context);
                context = None;
                index += increment;
            }
...
fn test_tokenize() {
    ...
    assert_eq!(evaluate(&tokens).1, 26);

    let tokens = tokenize("5 + (8 * 3 + 9 + 3 * 4 * 3)");
    assert_eq!(evaluate(&tokens).1, 437);

    let tokens = tokenize("5 * 9 * (7 * 3 * 3 + 9 * 3 + (8 + 6 * 4))");
    assert_eq!(evaluate(&tokens).1, 12240);

    println!("last one");
    let tokens = tokenize("((2 + 4 * 9) * (6 + 9 * 8 + 6) + 6) + 2 + 4 * 2");
    assert_eq!(evaluate(&tokens).1, 13632);
}
